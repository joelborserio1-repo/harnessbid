...
/*if($_SERVER['REMOTE_ADDR'] == '101.98.195.134') {
	echo "<a href='https://breedersbible.com/wwp-api/go.aspx?horse=global%20fenix&code=A019&token=845EB9A0-A344-4A7D-907E-956409258C00' target='_blank'>Test Horse</a>";
}*/
...
if(file_exists($tpl_profile_path_abs."style.css")) {
	$zulu->template->css_file[] = $zulu->path_clean($tpl_profile_path."style.css?6");
}
...
$BROWSE_sort_array = [
	...
	"2"	=> ['label'=>"Lowest price",'sort'=>'CASE WHEN price_special>0 THEN price_special ELSE price END ASC, add_feature DESC'],
	"3"	=> ['label'=>"Highest price",'sort'=>'CASE WHEN price_special>0 THEN price_special ELSE price END DESC, add_feature DESC'],
...
if(PAGE_file=='new_listing_type') {
	...
	if(!CLIENT_subscribed) {
		$listing_fee_label = "<br />Your listing fee: $".number_format($listing_price, 2)."<br /><i class=\"color-gold\">Premier Listing Fee: ".number_format($price_list->price_subscribe, 2)."</i>";
	} else {
		$listing_fee_label = "<br />Your <b>discounted</b> listing fee: $".number_format($listing_price, 2);
	}

	$zulu->template->jquery_code[] = "
	$('.listing-radio-selector').click(function() {
		let radio = $(this).find('input[type=\"radio\"]');
		$('.listing-radio-selector.selected').removeClass('selected');
		$(this).addClass('selected');
		radio.attr('checked', 'checked');
		radio.prop('checked', true);

		".(CLIENT_subscribed?"$(this).closest('form').submit(); //-- Automatically redirect when they click (members only - don't need to prompt for membership)":NULL)."
	});
	";
	...
	if(isset($_POST['action']) && $_POST['action'] == 'submit') {
		...
		if($form_edit->validate(['name'])) {
			$form_edit->valid = false;
			$zulu->notification_set('Please enter the name of the horse.', 2);

		} elseif($form_edit->validate(['spec_gait', 'spec_sire', 'spec_dam', 'spec_sex', 'spec_colour', 'spec_age'])) {
			$form_edit->valid = false;
			...
			$form_edit->valid = false;
			$zulu->notification_set('Please enter the location and region for the horse.', 2);
		...
		if($form_edit->valid) {
			...
			$_SESSION['LISTING']['spec_gait'] = $_POST['spec_gait'];
			$_SESSION['LISTING']['spec_sire'] = $_POST['spec_sire'];
			$_SESSION['LISTING']['spec_dam'] = $_POST['spec_dam'];
			$_SESSION['LISTING']['spec_sex'] = $_POST['spec_sex'];
			$_SESSION['LISTING']['spec_colour'] = $_POST['spec_colour'];
			$_SESSION['LISTING']['spec_age'] = $_POST['spec_age'];
			...
			if(!$is_edit) {
				if($_POST['date_close_fixed'] != 'Custom') {
					$_SESSION['LISTING']['date_close'] = strtotime('+'.$_POST['date_close_fixed']);
				} else {
					$_SESSION['LISTING']['date_close'] = strtotime($_POST['date_close_custom']);
				}
	...
	if($_POST) {
		...
		if($form_edit->valid) {
			...
			$product_meta = [
				'pedigree_link'		=>	$_SESSION['LISTING']['meta']['pedigree_link'],
			];
			...
			if(!$is_edit) {
				...
				$listing_data = [
					...
					'time_start'	=>	time(),
					'time_close'	=>	strtotime($_SESSION['LISTING']['time_close'], $_SESSION['LISTING']['date_close']),
					'time_close_schedule'	=>	strtotime($_SESSION['LISTING']['time_close'], $_SESSION['LISTING']['date_close']),
					'fee_listing'	=>	$fee,
					'fee_addon'		=>	$fee_addon,
					'price'			=>	$_SESSION['LISTING']['price'],
					'client_subscribed'		=>	0,
...
if(PAGE_file=='product') {
	...
	$bbURL = "https://breedersbible.com/wwp-api/go.aspx?horse=".urlencode($main_title)."&code=A019&token=845EB9A0-A344-4A7D-907E-956409258C00";
...
if(PAGE_file=='search') {
	...
	$row_PROD = $zulu->table_data($class_product->SQL_table_product, 0, [
		...
		'limit'	=>	$limit,
		'group'	=>	'product.id',
		//'test'	=>	$_SERVER['REMOTE_ADDR'] == '150.107.173.224' ? true : false,
	]);
...
if(PAGE_file=='update') {
	...
	if(!$_POST) {
		...
		//print_r($_POST);exit;
		$form_edit->valid = true;
		if($form_edit->validate(['name_first','name_last','email','timezone'])) {
...
if(PAGE_file=='register') {
	...
	if($_POST) {
		...
		$form_edit->valid = true;
        if($form_edit->validate(['name_first','name_last','email','password','password_c','location_id'])) {
...
//-- Fancybox
$zulu->template->css_file['jquery-fancybox'] = FE_tpl_rel."assets/fancybox/source/jquery.fancybox.css";
