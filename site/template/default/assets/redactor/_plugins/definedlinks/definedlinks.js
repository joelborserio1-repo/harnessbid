(function($R)
{
    $R.add('plugin', 'definedlinks', {
		...
		{
            if(this.opts.definedlinkTypes != null && this.opts.definedlinkTypes.length > 0) {
                var linkTypes = this.opts.definedlinkTypes;
                    
                var typeBody = this.$modal.getBody();
                var typeItem = $R.dom('<div class="form-item" />');
                var typeLabel = $R.dom('<label>Choose from a</label>');
                var typeSelector = $R.dom('<select id="redactor-defined-links-type" />');
                typeItem.append(typeLabel);
                typeItem.append(typeSelector);
                typeBody.prepend(typeItem);
                ...
                {
                    var $option = $R.dom('<option>');
                    $option.val(linkTypes[key].key);
                    $option.html(linkTypes[key].label);
                    typeSelector.append($option);
                }
